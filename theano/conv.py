...
def shallow(n=1, epochs=5, mini_batch_size=20,lmbda=0.1):
    nets = []
    for j in range(n):
        print "A deep net with 100 hidden neurons"
        net = Network([
            FullyConnectedLayer(n_in=784, n_out=300),
            FullyConnectedLayer(n_in=300, n_out=100),
            SoftmaxLayer(n_in=100, n_out=10)], mini_batch_size)
        net.SGD(
            training_data, epochs, mini_batch_size, lmbda, 
            validation_data, test_data)
        nets.append(net)
    return nets 
...
def expanded_data(n=100):
    ...
    n=100, 300, and 1000.
    """
    expanded_training_data, _, _ = network.load_data_mnist(
